...
public record LoanVO (int id,String userName, int aadhar, int cibilScore, String loanType, String asset, int assetValue,
	...
	public static LoanVO from(Loan loan) {
		return new LoanVO(loan.getId(),loan.getUserName(), loan.getAadhar(), loan.getCibilScore(), loan.getLoanType().name(), loan.getAsset(), loan.getAssetValue(), loan.getValue(),loan.getIncome(),loan.getLoanStatus().name() );
	}
