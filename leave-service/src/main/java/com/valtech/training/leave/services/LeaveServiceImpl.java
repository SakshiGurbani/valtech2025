...
import org.springframework.transaction.annotation.Transactional;import org.springframework.transaction.event.TransactionPhase;
...
public class LeaveServiceImpl implements LeaveService {
	...
	@Override
	public List<LeaveVO> getLeaveByEmployee(long emp){
		List<Leave>leaves=leaveRepo.findAllByEmployeeId(emp);
		return LeaveVO.to(leaves);
	}
	
	@Override
	public List<LeaveVO> getAllLeave(){
	...
	public List<LeaveMasterVO> getAllLeaveMasters(){
		return leaveMasterRepo.findAll().stream()
				.map(lm->LeaveMasterVO.from(lm)).collect(Collectors.toList());
